...
<Callout mark="Why">
  Environment variables are an *input boundary*, just like an HTTP request body.
  The rule at every boundary is the same: **parse, don’t cast.** Validate the
  untrusted shape into a trusted one, once.
</Callout>
...
- Your editor now _knows_ `config.port` is a `number` — no casting, no guessing.

<Figure
  ratio="16/8"
  caption="Fig 1. Untrusted env in, frozen typed config out — the boundary crossed exactly once."
/>
...
<Terminal
  ...
  intro={[
    "[muted]# the image expects DATABASE_URL, PORT, DEBUG. try 'docker run app'[/muted]",
  ]}
  demo="docker run app"
  commands={{
    'docker run app': [
...
<Callout kind="note" mark="Tip">
  Keep the parser dependency-free in small projects. When your schema grows past
  a dozen fields or needs nested shapes, reach for a runtime validator — but the
  *boundary* stays in exactly this one file.
</Callout>
