			...
			const key = Object.keys(commands).find(
				(k) =>
					...
					(cmd.startsWith(k.split(' ')[0] + ' ') &&
						k.split(' ').slice(0, 2).join(' ') === cmd.split(' ').slice(0, 2).join(' ')),
			);
			if (key) out = commands[key];
		}
		if (!out) {
			print(`[err]${cmd.split(' ')[0]}: command not found[/err] [muted]— type 'help'[/muted]`);
			return;
		}
...
<div class="term">
	<div class="tm-bar">
		...
		<span class="sp"></span>
		{#if demo}
			<button class="tm-run" type="button" onclick={playDemo}>▶ Play demo</button>
		{/if}
	</div>
	<div class="tm-body" bind:this={bodyEl}>
		{#each lines as line}
			<div class="tm-line">
				{#each line as seg}{#if seg.cls}<span class={seg.cls}>{seg.text}</span
